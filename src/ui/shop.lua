...
function Shop.draw(gameState, fonts)
	...
	local windowX = (Constants.General.WINDOW_WIDTH - Constants.Shop.POPUP_WIDTH) / 2
	local windowY = (Constants.General.WINDOW_HEIGHT - Constants.Shop.POPUP_HEIGHT) / 2
	...
	love.graphics.rectangle("fill", windowX, windowY, Constants.Shop.POPUP_WIDTH, Constants.Shop.POPUP_HEIGHT, 10)

	-- Draw title and available coins
	love.graphics.setFont(fonts.title)
	love.graphics.setColor(1, 1, 1, 1)
	love.graphics.printf("Pet Actions", windowX, windowY + 10, Constants.Shop.POPUP_WIDTH, "center")

	love.graphics.setFont(fonts.button)
	love.graphics.setColor(unpack(Colors.coins))
	love.graphics.printf("Coins $" .. gameState.pet.coins, windowX, windowY + 45, Constants.Shop.POPUP_WIDTH, "center")
	...
	for i, action in ipairs(Shop.actions) do
		...
		local x = windowX
			+ Constants.Shop.POPUP_WIDTH / 2
			+ (col - 0.5) * Constants.Shop.RADIO_SPACING_X
			- Constants.Shop.RADIO_SIZE
		local y = startY + row * Constants.Shop.RADIO_SPACING_Y + 10
		...
		-- Draw icon
		local iconScale = Constants.Shop.ICON_SIZE / action.icon:getWidth()
		love.graphics.draw(
			action.icon,
			x - Constants.Shop.ICON_SIZE - Constants.Shop.ICON_PADDING,
			y - Constants.Shop.ICON_SIZE / 3,
			0,
			iconScale,
			iconScale
		)
		...
		-- Radio circle
		love.graphics.circle("line", x + Constants.Shop.RADIO_SIZE, y, Constants.Shop.RADIO_SIZE)
		if selectedAction == action.action then
			love.graphics.circle("fill", x + Constants.Shop.RADIO_SIZE, y, Constants.Shop.RADIO_SIZE - 3)
		end

		-- Action name
		love.graphics.print(action.text, x, y + Constants.Shop.RADIO_SIZE + 5)

		-- Draw cost and coin icon in aligned columns
		love.graphics.setColor(unpack(Colors.coins))
		local costX = x + Constants.Shop.COIN_COLUMN_OFFSET
		love.graphics.print(gameState.pet.action_costs[action.action], costX, y + Constants.Shop.RADIO_SIZE + 5)

		-- Draw coin icon (aligned in column)
		love.graphics.draw(
			coinIcon,
			costX + fonts.button:getWidth(tostring(gameState.pet.action_costs[action.action])) + 5,
			y + Constants.Shop.RADIO_SIZE + 6,
			0,
			coinScale,
			coinScale
		)
	...
	if selectedAction then
		...
		love.graphics.printf(
			effectsText,
			windowX,
			windowY + Constants.Shop.POPUP_HEIGHT - 100,
			Constants.Shop.POPUP_WIDTH,
			"center"
		)
		...
		local centerX = windowX + Constants.Shop.POPUP_WIDTH / 2

		love.graphics.print(costText, centerX - textWidth / 2 - 5, windowY + Constants.Shop.POPUP_HEIGHT - 75)
		love.graphics.draw(
			coinIcon,
			centerX + textWidth / 2,
			windowY + Constants.Shop.POPUP_HEIGHT - 73,
			0,
			coinScale,
			coinScale
		)
	...
	-- Buy button
	local buttonX = windowX + (Constants.Shop.POPUP_WIDTH - Constants.Shop.BUY_BUTTON.width) / 2
	local buttonY = windowY + Constants.Shop.POPUP_HEIGHT - Constants.Shop.BUY_BUTTON.height - 20
	...
	love.graphics.rectangle(
		...
		buttonX,
		buttonY,
		Constants.Shop.BUY_BUTTON.width,
		Constants.Shop.BUY_BUTTON.height,
		5
	)
	love.graphics.setColor(1, 1, 1, canBuy and 1 or 0.5)
	love.graphics.printf("Buy", buttonX, buttonY + 5, Constants.Shop.BUY_BUTTON.width, "center")

	-- Close button
	love.graphics.setFont(fonts.close)
	love.graphics.setColor(1, 1, 1, 0.8)
	local closeX = windowX + Constants.Shop.POPUP_WIDTH - 30
...
function Shop.handleClick(x, y, gameState)
	...
	for i, action in ipairs(Shop.actions) do
		...
		local radioX = windowX
			+ Constants.Shop.POPUP_WIDTH / 2
			+ (col - 0.5) * Constants.Shop.RADIO_SPACING_X
			- Constants.Shop.RADIO_SIZE
		local radioY = startY + row * Constants.Shop.RADIO_SPACING_Y + 10

		local dx = x - (radioX + Constants.Shop.RADIO_SIZE)
		local dy = y - radioY

		if dx * dx + dy * dy <= Constants.Shop.RADIO_SIZE * Constants.Shop.RADIO_SIZE then
	...
	if
		selectedAction
		and x >= buttonX
		and x <= buttonX + Constants.Shop.BUY_BUTTON.width
		and y >= buttonY
		and y <= buttonY + Constants.Shop.BUY_BUTTON.height
	then
